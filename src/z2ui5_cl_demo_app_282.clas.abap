  ...
  METHOD display_view.
    ...
    page_02 = page_01->page(
                              ...
                              class = `sapUiContentPadding`
                                      ...
                                      )->button( icon = `sap-icon://edit` press = client->_event( val = `onPress` t_arg = temp2 ) arialabelledby = `editButtonLabel`
                                  )->get_parent(
                                      ...
                                      )->button( icon = `sap-icon://action` press = client->_event( val = `onPress` t_arg = temp5 ) arialabelledby = `actionButtonLabel`
                                      )->toolbar_spacer(
                                  )->get_parent(
                                  ...
                                  " These labels exist only to provide targets for the ARIA label on the Edit and Action buttons
                                      ...
                                      )->button( icon = `sap-icon://action` press = client->_event( val = `onPress` t_arg = temp12 )
                                  )->get_parent(
                              )->get_parent(
                             ).
